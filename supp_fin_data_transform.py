...
# func to add region to rows based on port
...
agriculture_df = agriculture_df.replace('Varsinais-Suomi', 'Southwest Finland')
...
air_travel_df = create_df('airplane_fin')
air_travel_df = conv_year(air_travel_df)
air_travel_df = assign_region(air_travel_df, 'airport', airport_reg_dict)
...
wood_consumption_df = wood_consumption_df.replace(
    'Varsinais-Suomi', 'Southwest Finland')
wood_consumption_df = wood_consumption_df.replace('Etelae-Savo', 'South Savo')
wood_consumption_df = wood_consumption_df.replace('Pohjois-Savo', 'North Savo')
...
air_travel_df.to_csv(OUTPUT_PATH + 'air_passenger_and_cargo_transport_fin.csv', index=False)
agriculture_df.to_csv(OUTPUT_PATH + 'agriculture_fin.csv', index=False)
consumption_population_df.to_csv(OUTPUT_PATH + 'energy_consumption_and_population_fin.csv', index=False)
